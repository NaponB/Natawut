...
<div class="container-fluid">
  <div class="row content">
    <div class="col-sm-2 sidenav hidden-xs"><br>
      <i href="#" class="a"><img class="home" src="img/user.png">&nbsp;&nbsp; ผู้ใช้ระบบ : Thararat</i><hr>
    ...
    <br>
      ...
      <div class="col-sm-10">
        <div class="col-md-8">
          <h4><small>ข้อมูลขนาดอาหาร</small></h4>
        </div>
        <div class="col-md-4">
        ...
        <div class="row">
          <div class="col-sm-3">
            <div class="well">
             <?
              @include('conn.php');
              $new_id =mysql_result(mysql_query("Select Max(substr(size_id,-2))+1 as MaxID from  size"),0,"MaxID");
              if($new_id==''){ // ถ้าได้เป็นค่าว่าง หรือ null ก็แสดงว่ายังไม่มีข้อมูลในฐานข้อมูล
                    $size_id="S01";
                }else{
                    $size_id="S".sprintf("%02d",$new_id);//ถ้าไม่ใช่ค่าว่าง
                }
          ?>
              <label for="">รหัสขนาดอาหาร</label>
              <input type="text" class="form-control" id="size_id" value="<?echo $size_id;?>" disabled>
              <br>
              <label for="">ชื่อขนาดอาหาร  &nbsp;&nbsp;&nbsp; <img class="asterisk" src="img/asterisk.png"></label>
            ...
            <div class="well">
              <table class="table table-hover">
                ...
                <tbody>
                 <?
                @include('conn.php');
                  $strSQL = "SELECT  * FROM size";
                  $objQuery = mysql_query($strSQL) or die("Error Query [".$strSQL."]");
                  while ($objReSult = mysql_fetch_array($objQuery)) {
              ?>
                  <tr>
                    <td><?echo $objReSult['size_id'];?></td>
                    <td><?echo $objReSult['size_name'];?></td>
                    <td>
                    <button type="button" class="btn btn-info" data-toggle="tooltip"  title="แก้ไขข้อมูล" onclick="updateId(<? echo "'".$objReSult["size_id"]."'";?>)"><img class="plus" src="img/edit.png"></button>
                    <button type="button" class="btn btn-warning" data-toggle="tooltip"  title="ลบข้อมูล" onclick="delData(<? echo "'".$objReSult["size_id"]."'";?>)"><img class="plus" src="img/delete.png"></button>
                    </td>
                  </tr>
                  <?
                  }?>
